...
def get_stock_info(code):
    return str(stock_db.find_one({'code': code}))

# 주식 리스트를 불러오는 메소드


def get_stock_list():
    # print(type(json.dumps(list(list_db.find()))))

    return str(list(list_db.find()))
...
def get_stock_sort(roe, eps, debt):
    ...
    for l in lists:
        cd = l['code']
        share = stock_cal(cd)
        stock = {"name": l['name'], "share": share, "rim": l['rim'], "per":  str(Per_cal(
